...
WITH vendas AS (
    ...
    JOIN {{ ref('dim_veiculos') }} AS vei ON v.id_veiculos = vei.veiculo_id
    JOIN {{ ref('dim_concessionarias') }} AS con ON v.id_concessionarias = con.concessionaria_id
    JOIN {{ ref('dim_vendedores') }} AS ven ON v.id_vendedores = ven.vendedor_id
    JOIN {{ ref('dim_clientes') }} AS cli ON v.id_clientes = cli.cliente_id
)
